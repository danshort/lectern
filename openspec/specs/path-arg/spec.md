...
## Purpose
Allows launching `speclio` with an explicit path to a specific change, bypassing the scan of `openspec/changes/` and limiting polling to that single change.
...
- **WHEN** the user runs `./speclio` with no arguments
...
- **WHEN** the user runs `./speclio ./openspec/changes/mi-change`
- **THEN** the TUI shows only the artifacts of `mi-change`

#### Scenario: Invocación con ruta a change archivado
- **WHEN** the user runs `./speclio ./openspec/changes/archive/2026-05-02-mi-change`
...
- **WHEN** the user runs `./speclio ./ruta/que/no/existe`
- **THEN** the binary prints `"error: path not found: ./ruta/que/no/existe"` and exits with code 1

#### Scenario: Ruta sin .openspec.yaml
- **WHEN** the user runs `./speclio ./algún/directorio` and that directory does not contain `.openspec.yaml`
